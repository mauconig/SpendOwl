...
export type ApiTransaction = {
  ...
  cardId: string | null;
  /**
   * Which card this row paid *down* — set only on a card payment, and never at
   * the same time as `cardId`. The two are opposites: `cardId` means the
   * account has not been debited yet, this means it just was. Deleting the row
   * gives the debt back, which is the reason it is recorded at all.
   */
...
export type ApiMessage = {
  ...
  payload: {
    ...
    cardId?: string;
    cardName?: string;
    /** sub_cancel, sub_delete, sub_add and sub_edit. */
    subId?: string;
    subName?: string;
    dayOfMonth?: number;
    /**
